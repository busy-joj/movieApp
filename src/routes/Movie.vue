...
.skeletons{
  ...
  .poster{
    flex-shrink: 0;
    width: 500px;height:500px *3/2;margin-right:70px;position:relative;
  }
  .specs{
    flex-grow:1;
  }
  .skeleton{border-radius: 10px;background-color: $gray-200;
    &.title{
...
.movie-details{display:flex;color:$gray-600;
  ...
  .specs{flex-grow:1;
    ...
    .labels{color:$primary;
      span{
        &::after{content:'\00b7';margin:0 6px;}
        &:last-child::after{display:none;}
      }
    }
    .plot{margin-top:20px;}
    .ratings{
      .rating-wrap{display: flex;
        .rating{display: flex;align-items:center;margin-right:32px;
          img{height:30px;flex-shrink: 0;margin-right:6px;}
        }
      }
      }
